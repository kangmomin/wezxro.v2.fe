...
function callPostAjax(element, url, data, type, redirect = null) {
    ...
    $.ajax({
        url: host + url,
        data: JSON.stringify(formDataToJson(data)),
        method: "post",
        headers: {
        ...
        success: function (_result) {
            ...
            if (_result.status === "ERROR") return notify(_result.data.message, "error")
            switch (type) {
...
function formDataToJson(formDataString) {
    // '='를 기준으로 폼 데이터를 분할하여 배열로 만듭니다.
    let formDataArray = formDataString.split('&');

    // 결과를 저장할 JSON 객체 생성
    let jsonData = {};

    // 배열을 순회하면서 키와 값을 JSON 객체에 추가합니다.
    formDataArray.forEach(function(pair) {
        let keyValue = pair.split('=');
        let key = keyValue[0];
        let value = decodeURIComponent(keyValue[1].replace(/\+/g, ' '));
